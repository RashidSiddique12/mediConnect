...
import { fetchReviews, submitReview, approveReview, rejectReview } from '@/services/api'

function* handleFetchReviews(action) {
  try {
    const response = yield call(fetchReviews, action.payload)
    yield put(reviewSlice.fetchReviewsSuccess(response.data.data || []))
  } catch (error) {
...
export function* watchReviewSaga() {
  ...
  yield takeLatest(reviewSlice.moderateReviewRequest.type, handleModerateReview)
}
