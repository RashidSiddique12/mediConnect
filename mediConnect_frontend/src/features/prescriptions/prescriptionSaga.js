...
function* handleFetchPrescriptions(action) {
  try {
    ...
    yield put(prescriptionSlice.fetchPrescriptionsSuccess(response.data.data || []))
  } catch (error) {
...
function* handleFetchPrescriptionByAppointment(action) {
  try {
    ...
    yield put(prescriptionSlice.fetchPrescriptionByAppointmentSuccess(response.data.data))
  } catch (error) {
    yield put(prescriptionSlice.fetchPrescriptionByAppointmentFailure(error.message))
  }
}
...
export function* watchPrescriptionSaga() {
  yield takeLatest(prescriptionSlice.fetchPrescriptionsRequest.type, handleFetchPrescriptions)
  yield takeLatest(prescriptionSlice.fetchPrescriptionByAppointmentRequest.type, handleFetchPrescriptionByAppointment)
  yield takeLatest(prescriptionSlice.uploadPrescriptionRequest.type, handleUploadPrescription)
}
