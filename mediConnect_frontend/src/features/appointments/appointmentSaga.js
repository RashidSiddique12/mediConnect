...
function* handleFetchAppointments(action) {
  try {
    ...
    yield put(appointmentSlice.fetchAppointmentsSuccess(response.data.data || []))
  } catch (error) {
...
export function* watchAppointmentSaga() {
  yield takeLatest(appointmentSlice.fetchAppointmentsRequest.type, handleFetchAppointments)
  yield takeLatest(appointmentSlice.fetchAppointmentByIdRequest.type, handleFetchAppointmentById)
  yield takeLatest(appointmentSlice.bookAppointmentRequest.type, handleBookAppointment)
  yield takeLatest(appointmentSlice.cancelAppointmentRequest.type, handleCancelAppointment)
  yield takeLatest(appointmentSlice.updateAppointmentRequest.type, handleUpdateAppointment)
}
