...
export const MOCK_REVIEWS = [
  ...
  },
  {
    ...
    hospitalId: "h-002",
    rating: 3,
    comment: "Long wait time but good treatment.",
    date: "2026-04-18",
    status: "pending",
  },
];
...
const MOCK_DASHBOARD = {
  ...
  hospital_admin: {
    ...
    recentAppointments: MOCK_APPOINTMENTS.filter(
      (a) => a.hospitalId === "h-001",
    ),
  ...
  patient: {
    stats: {
      upcomingAppointments: MOCK_APPOINTMENTS.filter(
        (a) => a.patientId === "u-003" && a.status !== "cancelled",
      ).length,
      completedAppointments: MOCK_APPOINTMENTS.filter(
        (a) => a.patientId === "u-003" && a.status === "completed",
      ).length,
...
export const mockFetchReviews = async (filters = {}) => {
  ...
  if (filters.patientId)
    list = list.filter((r) => r.patientId === filters.patientId);
  return { data: list };
};
...
export const MOCK_USERS_LIST = [
  ...
  {
    id: "u-006",
    name: "Sunrise Medical Admin",
    email: "sunrise@healthcare.com",
    role: "hospital_admin",
    status: "active",
